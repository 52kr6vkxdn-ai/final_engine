...
const COMPLETIONS = [
    ...
    // Game helpers
    // removed: { n:'gravity',           m:'🎮 game',     v:'gravity(${1:vy},
    // removed: { n:'launch',            m:'🎮 game',     v:'launch(${1:vx}, 
    ...
    // removed: { n:'setGravityScale',   m:'⚙ physics',  v:'setGravityScale(
    // Extra math
...
function _sidebarHTML() {
    const G = [
        ...
        ['Spawn / Clone', [
            ...
            'obj.x = 5  obj.rotation = 45  obj.alpha = 0.5',
            'var bullet = spawnObject("Bullet", x, y)',
            'bullet.velocityX = 10',
            'spawnObject("Asset", x, y, (obj) => {})',
            'spawnCopy("Name", x, y)',
            'var clone = cloneSelf(x, y)',
            'clone.rotation = 180  clone.scaleX = -1',
            'cloneSelf(x, y, (c) => { c.velocityX = 3; })',
            'cloneInPlace()',
            'var enemy = cloneObject("Enemy", x, y)',
            'enemy.setVelocity(3, 0)',
            'isClone()',
            'getCloneId()',
            'onCloneStart(() => { })',
            '// proxy properties: x y rotation scaleX scaleY',
            '// alpha visible velocityX velocityY tag group',
            '// health ammo state zOrder opts text',
        ]],
        ['Physics (helpers)', [
...
// GRAVITY:       setGravityScale(1.0)   (change gravity strength on physics bodies)
...
onStart(() => {
  ...
  // setGravityScale(1.0);  ← adjust physics gravity scale
