...
const COMPLETIONS = [
    ...
    // ── Grab / Drag / Throw (unified) ──────────────────────
    { n:'makeGrabbable',           m:'grab',  v:'makeGrabbable()' },
    { n:'makeGrabbable place',     m:'grab',  v:"makeGrabbable({ smooth:${1:16}, clamp:${2:true}, scale:${3:1.08} })" },
    { n:'makeGrabbable throw',     m:'grab',  v:"makeGrabbable({ throw:true, speed:${1:1}, maxSpeed:${2:25} })" },
    { n:'makeGrabbable onDrop',    m:'grab',  v:"makeGrabbable({ onDrop:(x,y) => { ${1:log(x,y)} } })" },
    { n:'makeGrabbable onThrow',   m:'grab',  v:"makeGrabbable({ throw:true, onThrow:(vx,vy) => { ${1:log(vx,vy)} } })" },
    { n:'grabObject',              m:'grab',  v:"grabObject(find('${1:Crate}'))" },
    { n:'grabObject throw',        m:'grab',  v:"grabObject(find('${1:Ball}'), { throw:true, speed:${2:1.5} })" },
    { n:'stopDrag',                m:'grab',  v:'stopDrag()' },
    { n:'isDragging',              m:'grab',  v:'isDragging()' },
    // aliases
...
function _sidebarHTML() {
    ...
    const G = [
        ...
        ['Grab / Drag / Throw', [
            '// makeGrabbable — unified API, works on dynamic + kinematic bodies',
            '// Dynamic bodies use a Planck MouseJoint: collisions block the drag,',
            '// velocity builds up naturally — same as Unity/Godot/Box2D demos.',
            'onStart(() => { makeGrabbable(); })                          // drag and place',
            'onStart(() => { makeGrabbable({ throw: true }); })          // drag and throw',
            'onStart(() => { makeGrabbable({ throw:true, speed:1.4, maxSpeed:25 }); })',
            'onStart(() => { makeGrabbable({ clamp:true, scale:1.1 }); })',
            'onStart(() => { makeGrabbable({ onDrop:(x,y) => { log(x,y) } }); })',
            'onStart(() => { makeGrabbable({ throw:true, onThrow:(vx,vy) => { log(vx,vy) } }); })',
            '// Low-level: start grab now (call from onMouseClick or mouseJustDown)',
            'grabObject()                          // grab self',
            ...
            'stopDrag()                            // release current grab',
            'isDragging()                          // true while held',
            '// Aliases (backwards compatible)',
            'makeDraggable()   // = makeGrabbable()',
            'makeThrowable()   // = makeGrabbable({ throw: true })',
            'dragObject(...)   // = grabObject(...)',
            'throwObject(...)  // = grabObject(..., { throw: true })',
        ]],
        ['Animation', [
            'playAnimation("name")',
            'stopAnimation()',
